...
class BookingPageController extends ChangeNotifier {
  BookingPageController(
      {required this.ref, required this.date, required this.time});
  ...
  bookDate(int patientId, int doctorId, BuildContext context) async {
    ...
    final res = await remoteRepo.bookDate(date, time, patient!.id, doctor!.id);
    res.fold((l) {
      showSnackBar(context, l.toString());
    }, (r) => showSnackBar(context, "Booking is successful"));
...
final bookingPageProvider =
  ...
  return BookingPageController(
      ref: ref, date: DateTime.now(), time: TimeOfDay.now());
});
